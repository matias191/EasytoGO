...
<html>
	...
	<body>
		...
		<div class="col-md-9">
		<h1>Añade tu foto!</h1>
  		<h5>Es importante que actualices tu foto de perfil para facilitar el encuentro a la hora de viajar.</h5>
  
 	 <g:uploadForm action="upload_avatar">
 	   <div class="row">
 	   <label for="avatar">Selecciona una foto (1 MB Máx.)</label>
  	  <div class = "btn-group  ">
  	  <input type="file" class="btn btn-sm" name="avatar" id="avatar" placeholder="Seleccionar archivo"/>
       </div>	
       </div>
        <div class="row">
 	   <label for="avatar">Añadir al perfil</label>
        <div class = "btn-group  ">
  	  	<button type="submit" class="btn btn-sm" name="añadir" value="Añadir" >Añadir</button>
  	     </div>	
  	     </div>
  		</g:uploadForm>
		</div>	
			...
			<g:form url="[resource:userInstance, action:'update']" method="PUT" class="form-horizontal">
				...
				<div class="container">
					...
					<g:link class="save btn btn-success btn-lg"controller="vehiculo" action="create" params="['user.id': userInstance?.id]">${message(code: 'default.add.label', args: [message(code: 'vehiculo.label', default: 'Vehiculo')])}</g:link>
					</div>
					</div>
    			</div>	
				</fieldset>
